						...
						<div id="edicao" >
							<form id="frm_edicao" method="post">
								<div id="dv_nome">
									...
									<input class="cl_edicao_nome" type="text" maxlength="100" name="nome" id="nome" value="{$admin->nome}" />																
								</div>
								<div id="dv_email">
									<label class="lb_edicao_email"><font size="4" color="#32639A">Email: </font></label>								
									<input class="cl_edicao_email" type="text" maxlength="100" name="email" id="email" value="{$admin->email_usuario}" />
								</div>
								<div id="dv_estado">
									...
									<select class="cl_edicao_estado" name="estado" id="estado" onchange="atualizarCidades();">
										...
										{foreach from=$listaEstados item='estado'}
											<option value="{$estado->id}" {if $estado_admin->id == $estado->id}selected="selected"{/if}>{$estado->nome}</option>
										{/foreach}
									</select>
								</div>
								<div id="dv_cidade">
									...
									<select class="cl_edicao_cidade" name="cidade" id="cidade">								
										...
										{foreach from=$listaCidades item='cidade'}
											<option value="{$cidade->id}" {if $admin->cod_cidade == $cidade->id}selected="selected"{/if}>{$cidade->nome}</option>
										{/foreach}
									</select>
								</div>								
								<div id="dv_cep">
									<label class="lb_edicao_cep"><font size="4" color="#32639A">CEP: </font></label>
									<input class="cl_edicao_cep" type="text" maxlength="20" name="cep" id="cep" value="{$admin->cep}" />
								</div>								
								<div id="dv_sexo">
									<label class="lb_edicao_sexo"><font size="4" color="#32639A">Sexo: </font></label>
									<select class="cl_edicao_sexo" name="sexo" id="sexo" >
										<option value="">Selecione</option>
										<option value="Feminino" {if $admin->sexo == 'Feminino'}selected="selected"{/if}>Feminino</option>
										<option value="Masculino" {if $admin->sexo == 'Masculino'}selected="selected"{/if}>Masculino</option>
									</select>
								</div>
								<div id="dv_nasc">
									<label class="lb_edicao_dtnasc"><font size="4" color="#32639A">Data nascimento: </font></label>
									<input class="cl_edicao_dtnasc" type="text" maxlength="20" name="data_nasc" id="data_nasc" value="{$admin->data_nascimento}" />
								</div>
								<div id="dv_escolaridade">
									...
									<select class="cl_edicao_escolaridade" name="escolaridade" id="escolaridade">
										...
										{foreach from=$listaEscolaridade item='escolaridade'}
											<option value="{$escolaridade->id}" {if $admin->escolaridade == $escolaridade->id}selected="selected"{/if}>{$escolaridade->nivel}</option>
										{/foreach}
									</select>
								</div>
								<div id="dv_senha">
									<label class="lb_edicao_senha"><font size="4" color="#32639A">Senha: </font></label>
									<input class="cl_edicao_senha" type="password" maxlength="20" name="senha" id="senha" value="{$admin->senha}" />
								</div>
								<div id="dv_confSenha">
									<label class="lb_edicao_confsenha"><font size="4" color="#32639A">Confirme a Senha: </font></label>
									<input class="cl_edicao_confsenha" type="password" maxlength="20" name="conf_senha" id="conf_senha" value="{$admin->senha}" />
								</div>
								<input type="hidden" name="hdn_id_admin" value="{$admin_id}" />
